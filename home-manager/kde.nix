...
in {
  ...
  home.file = lib.mkIf immutable {
    ...
    "${configDir}/autostart/org.telegram.desktop".text = ''
      ...
      Type=Application
    '';
    "${configDir}/autostart/tailscale-systray.desktop".source = "${(pkgs.makeDesktopItem {
      categories = ["System"];
      comment = "The Tailscale systray ported to Linux.";
      desktopName = "Tailscale Systray";
      exec = "${pkgs.tailscale-systray}/bin/tailscale-systray";
      icon = "mullvad-vpn";
      name = "tailscale-systray";
    })}/share/applications/tailscale-systray.desktop";
